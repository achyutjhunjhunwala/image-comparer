...
compareImages.post('/compare-images-ignore-colors', [jsonParser, ...ImageComparePayloadValidation], async (req, res, next) => {
  try {
    validationResult(req).throw();
  } catch (err) {
    logger.info('Route:compare-images-ignore-colors, error %j', err.mapped());
  ...
  logger.info('Route:compare-images-ignore-colors, 1st image URL received: %j', imgUrl1);
  logger.info('Route:compare-images-ignore-colors, 2nd image URL received: %j', imgUrl2);

  try {
    const image1 = new ImageFetcher(imgUrl1);
    const image2 = new ImageFetcher(imgUrl2);

    logger.info('Route:compare-images-ignore-colors, Instance Created');

    let imagedata = await getImages(image1, image2);

    logger.info('Route:compare-images-ignore-colors, Image Fetched');
    ...
    logger.info(`Route:compare-images-ignore-colors, Something stupid happened for ${imgUrl1}, ${imgUrl2}`);
    next(err);
  }
});
